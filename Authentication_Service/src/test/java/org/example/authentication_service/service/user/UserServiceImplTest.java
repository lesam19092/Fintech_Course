...
class UserServiceImplTest {
    ...
    void resetPassword() throws MessagingException {
        ...
        request.setUserType("testInstance");

        User user = new User();
        user.setEmail("test@example.com");
        user.setEnable(true);

        when(userRepository.findByMailAndInstance("test@example.com", "testInstance")).thenReturn(Optional.of(user));
        when(passwordResetTokenService.generatePasswordResetToken(user)).thenReturn("resetToken");
        doNothing().when(emailService).sendEmailWithRestorePassword("test@example.com", "resetToken");

        userService.resetPassword(request);

        verify(userRepository, times(1)).findByMailAndInstance("test@example.com", "testInstance");
        verify(passwordResetTokenService, times(1)).generatePasswordResetToken(user);
        verify(emailService, times(1)).sendEmailWithRestorePassword("test@example.com", "resetToken");
    }
}